...
export const Exam = (props: IExamProps) => {
  ...
  const { examList, match, loading, totalItems } = props;
  return (
    <div>
      <h2 id="exam-heading">
        ...
        {examList && examList.length > 0 ? (
          <Table responsive>
            <thead>
              <tr>
                ...
                <th className="hand" onClick={sort('idExam')}>
            ...
            <tbody>
              {examList.map((exam, i) => (
                <tr key={`entity-${i}`}>
                  ...
                  <td>{exam.startDate ? <TextFormat type="date" value={exam.startDate} format={APP_DATE_FORMAT} /> : null}</td>
                  <td>{exam.endDate ? <TextFormat type="date" value={exam.endDate} format={APP_DATE_FORMAT} /> : null}</td>
                  <td className="text-right">
                    <div className="btn-group flex-btn-group-container">
                      ...
                      <Button
                        tag={Link}
                        to={`${match.url}/${exam.id}/edit?page=${paginationState.activePage}&sort=${paginationState.sort},${paginationState.order}`}
                        color="primary"
                        size="sm"
                      >
                      ...
                      <Button
                        tag={Link}
                        to={`${match.url}/${exam.id}/delete?page=${paginationState.activePage}&sort=${paginationState.sort},${paginationState.order}`}
                        color="danger"
                        size="sm"
                      >
          ...
          !loading && (
            <div className="alert alert-warning">
              <Translate contentKey="btbApp.examExam.home.notFound">No Exams found</Translate>
            </div>
          )
        )}
      </div>
